...
const wsLink = new GraphQLWsLink(
  createClient({
    url: (window.location.protocol === 'https:' ? 'wss' : 'ws') + (process.env.NODE_ENV === 'development' ? `://localhost:${process.env.REACT_APP_API_PORT}` : `://${window.location.host}`) + '/subscriptions',
    connectionParams: () => ({
